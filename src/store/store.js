...
import {checkDataFromStorage} from 'src/api/util'
...
export const getters = {
  ...
  searchDetailView: (state) => {
    return state.searchDetailView
  },
  iamChuckNorris: (state) => {
    return state.iamChuckNorris
  }
}
...
export const mutations = {
  ...
  setSearchDetailView (state, value) {
    state.searchDetailView = value
  }
}
...
export const actions = {
  getRandomJokes ({commit}) {
    ...
    api.getRandomJokes((response) => {
      ...
      console.log('fallback random jokes')
      commit('setLoading', false)
      let sessionRes = checkDataFromStorage(`${basePath}jokes/random`, false)
      if (sessionRes) commit('setRandomJokes', sessionRes.body)      
    }, null)
  },
